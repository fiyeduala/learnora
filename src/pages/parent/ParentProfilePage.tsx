...
const children = [
  { name: 'Camson Ashuma', grade: 'Grade A', attendance: '98%', performance: 'Good' },
  { name: 'Rachel Ashuma', grade: 'Grade A', attendance: '98%', performance: 'Good' },
]
    ...
    <MobileLayout activePage="parent/profile" onNavigate={onNavigate} nav={parentMobileNav}>
      <div className="px-5 pt-5 pb-6">
        ...
        <div className="bg-white border border-black/8 rounded-2xl p-4 mb-5 shadow-sm flex items-center gap-4">
          <div className="size-16 rounded-full bg-gray-200 flex items-center justify-center text-2xl shrink-0">👨</div>
          <div className="flex-1 min-w-0">
            <p className="text-base font-bold text-foreground">Olive Ashuma</p>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          {children.map(c => (
            <button key={c.name} onClick={() => onNavigate('parent/progress')} className="bg-white border border-black/8 rounded-2xl p-3 text-left shadow-sm">
              <div className="size-8 rounded-full bg-primary/15 flex items-center justify-center text-lg mb-2">👦</div>
              <p className="text-xs font-bold text-foreground">{c.name}</p>
              <p className="text-[10px] text-green-600 font-semibold">{c.grade}</p>
              <div className="flex justify-between mt-1">
                <div><p className="text-[9px] text-muted">Attendance</p><p className="text-xs font-bold text-green-600">{c.attendance}</p></div>
                <div><p className="text-[9px] text-muted">Performance</p><p className="text-xs font-bold text-green-600">{c.performance}</p></div>
              </div>
            </button>
          ))}
        </div>
          ...
          {settings.map(s => {
            ...
            return (
              <button key={s.label} onClick={() => onNavigate(s.page)} className="w-full flex items-center gap-3 px-4 py-3.5 text-left">
                <div className="size-8 rounded-full bg-canvas flex items-center justify-center shrink-0">
        ...
        <button
          ...
          className="w-full h-12 bg-primary text-white text-sm font-bold rounded-full"
        >
          Log Out
        </button>

...
// School bank details (populated from admin settings)
...
export default function SelectPaymentMethodPage({ onNavigate }: Props) {
  ...
  function handleProceed() {
    if (selected === 'offline') return // offline shows details inline
  ...
  return (
    <MobileLayout activePage="parent/fees" onNavigate={onNavigate} nav={parentMobileNav}>
      <div className="px-5 pt-5 pb-8 flex flex-col min-h-full">
        ...
        <p className="text-sm text-muted mb-2">Choose a payment method to pay <strong>{fmt(balance)}</strong></p>
